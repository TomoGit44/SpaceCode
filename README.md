...
| 種別 | 色 | HP | 速度 | 攻撃 | 報酬 | 出現 |
|---|---|---|---|---|---|---|
| basic | 赤 | 20 | 60 | 体当たり 15 | $5 | Phase 1 〜 |
| fast | オレンジ | 12 | 95 | 体当たり 12 | $7 | Phase 3 〜 |
| tank | 濃赤 | 55 | 38 | 体当たり 22 | $14 | Phase 5 |
| **sniper** | ミント | 25 | 45 | 弾 10 (射程 280 / 1.8s 毎) | $10 | Phase 1 〜 |
| **boss** | 紫 | 200 | 30 | 体当たり 45 | $50 | Phase 5 末尾のみ (撃破で SR ガチャ確定) |

体当たり敵 (basic/fast/tank/boss) は接触時にシアン + 白の電気スタン稲妻を放電。船は接触持続中 12 dps を受ける。
sniper は遠距離型で射程内に入ると停止して基地に弾を発射。船は素通りするが基地接触はしない。
...
- **オムニ・コア** — 装着で全 Ship / 基地 / 経済の stat を強化。**画面左上に固有アイコンで常時表示** (hover で詳細)。Run 開始時に 3 つのスターターコア (**強化攻撃 +50% / 省エネ -50% / 耐久 +50%**) が自動装着される。他の 6 コア (攻撃・推進・採掘・装甲・砲塔・賞金) は今後の入手経路実装予定
- **モジュール** — Ship 個別に装着 (ガトリング / 装甲 / スラスタ / ドリル / カーゴ / **衝角ブレード** = 体当たりで敵にダメージ、移動速度トレードオフ)
- **コードガチャ / モジュールガチャ** — 開封すると 3 候補から 1 つ選択
- **入手経路**: Phase クリア毎に 1 個ガチャ + fast/tank ドロップ + ボス撃破 SR 確定 + Phase 半数撃破時に $80 クレジット
...
│   ├── effects.ts         #   EffectSystem (オムニ・コア + モジュール 加算スタック)
│   ├── codePlacement.ts   #   ITEM_CODE 配置の真実源走査
│   ├── gacha.ts           #   ガチャ抽選 (drawGacha / Phase 報酬重み)
│   └── types/             #   omniCores / modules / itemCodes
