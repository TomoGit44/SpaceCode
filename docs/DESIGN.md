...
| **Phase 6** | アイテムシステム: レア度 4 段階 / `Inventory` (Run 揮発) / `EffectSystem` (オムニ・コア + モジュール加算スタック) / **アイテムコード** (ITEM_CODE = 条件 wrapper、配置真実源は Code ノード自身) / **ガチャ系統** + **ボス敵** (Phase 5 末尾、SR ガチャ確定) + **中間ドロップ** (Phase 半数撃破で $80 クレジット) + **編集画面のモジュール表示** | 🔧 進行中 (Step 0-9 完了、実プレイ後バランス調整が残作業) |
...
| 補追 (Phase 6 後) | **遠距離敵 sniper** (`behavior: 'shoot'`、attackRange 280 で停止 + 弾発射) + **体当たり敵強化** (damage ×1.5 + 電気スタンガン演出) + **体当たりモジュール** `mod_ram` (衝角ブレード、contactDps flat + moveSpeed マイナス) | ✅ 完了 (2026-05-25) |
| 補追 (Phase 6 後) | **オムニ・コア左上 Strip 化** (固有アイコン + 常時表示 + hover ツールチップ) + **3 新コア** (`core_attack_plus` / `core_efficiency` / `core_endurance`、Run 開始時 SR 装着) + **アイテムメニューから omni タブ撤去** + ShipStat `energyConsume` 追加 | ✅ 完了 (2026-05-25) |
...
- `heal(amount)` / 接触ダメージ等が `maxHp` を意識せず純粋な加減算で済む
...
### 6.31 オムニ・コア UI 刷新 + 3 新コア + アイテムメニューからタブ撤去 (Phase 6 後, 2026-05-25)

**経緯**: プレイヤー要望「オムニコアそれぞれにアイコン、画面左上に常時横一列表示、アイテムメニューからの (オムニコア) ボタン削除。新コア 3 種: 攻撃 +50% / エネルギー消費 -50% / HP +50%」。

**判断**:
- **3 新コアは Run 開始時に SR レアリティで自動装着** (スターター方式)。`core_attack_plus` / `core_efficiency` / `core_endurance`
- **既存 6 コアの入手経路は撤去** (元々アイテムメニュー内 debug ボタンのみで、本変更でそれも消滅)。コードは残置、将来の Phase 報酬等で再導入可能
- `ShipStat` に **`'energyConsume'`** を追加。デフォルト 1.0 の倍率扱い、コアで -0.5 → 0.5 倍消費 (移動消費 + 射撃消費)
- **`OmniCoreStrip`** UI コンポーネント新規。所持コアを左上に横一列、固有 Graphics アイコン + レア度ボーダー + 重複時 ×N バッジ + hover ツールチップ
- **`ItemInventoryScene` から omniCore タブ削除**: `CATEGORIES` / `IMPLEMENTED` / dispatch / debug grant / unused imports すべて整理

**理由**:
- **`energyConsume` を倍率にする**: 加算スタック (`base * (1 + Σ percent)`) の枠で `-0.5` → `0.5 倍消費` が自然。新ヘルパ不要、既存 `effects.shipStat` の経路で完結
- **スターター方式が要望に最も整合**: 「左上常時表示」「アイテムメニューから撤去」「3 種追加」を一貫させる最短ルート。入手経路問題と切り離せる
- **画像なしの Graphics シンボル**: 9 種それぞれに視覚的に区別できるシンボル (lightning / arrow / drill / shield / shield+cross / crosshair / $ / battery / lightning+sparks)。プロジェクト方針の「画像アセット不使用」を維持
- **strip は HUD 配下の独立コンポーネント**: 既存 `HUD` を太らせず疎結合に。`refresh()` API でインベントリ変化を外から通知

**実装上の判断**:
- スターター 3 コアは `rarityPercent` を全レアリティ同じ値 (0.5 / -0.5 / 0.5) にして「レアリティ非依存の固定効果」を表現。表示用 rarity は SR で色がつくが効果はレア度に関係なし
- 既存 6 コアの定義 (`OMNI_CORE_TYPES`) は残置 — 削除すると将来再導入時の書き直しコストが発生。Inventory に入っていない限り無影響
- `renderCoreDetail` (旧コアタブの右パネル) は撤去。詳細は strip の hover ツールチップに集約 (passive 効果なので操作不要)

**トレードオフ**:
- 既存 6 コアは「存在するけど取れない」状態に。将来の入手経路 (Phase 報酬 / 新ガチャ枠) 設計が宿題
- スターター 3 コアの恒常 +50%/-50%/+50% は強力 (攻撃 1.5 倍、移動 50 秒 → 100 秒、射撃 20 発 → 40 発、HP 30 → 45)。実プレイ後にコスト調整 or レア度別効果分けの検討余地

### 6.30 遠距離敵 sniper + 体当たり強化 + 体当たりモジュール (Phase 6 後, 2026-05-25)

**経緯**: プレイヤー要望「全敵が体当たりで単調 → 遠距離攻撃する敵を加えたい。体当たり敵には電気スタンガンっぽい演出 + 攻撃力上げ。プレイヤー側にも体当たり戦法を取れるモジュールを」。

**判断**:
- **敵の行動様式を `EnemyBehavior = 'charge' | 'shoot'` で分離**: 既存 4 種は全部 'charge'、新規 sniper は 'shoot'。`EnemyTypeStats` に optional な `behavior` / `attackRange` / `fireIntervalMs` / `bulletDamage` / `bulletSpeed` を追加し、各種を data-driven のまま拡張
- **`EnemyBullet` を独立クラス化**: 既存 `Bullet` は Enemy をホーミングする設計で、静的座標を追うように改造すると分岐が増えるため独立 (100 行未満)
- **電気スタン演出は `spawnElectricArc(scene, fromX, fromY, toX, toY)` ヘルパ**: 6 セグメントのジグザグ線 (法線方向にランダム偏移) を 110ms フェードアウトで描画。船接触ループ内では 18% の確率発火でフレーム連発を防ぎつつ「ちらつき = スタン感」を出す
- **体当たり damage ×1.5**: basic 10→15 / fast 8→12 / tank 15→22 / boss 30→45 / `ENEMY_VS_SHIP.contactDps` 8→12
- **モジュール `mod_ram`** (衝角ブレード): `ModuleStatTarget` に `'contactDps'` を追加 (extraShots と同じ非 ShipStat 特殊値)、効果は `kind: 'flat'`。trade-off で `moveSpeed` percent マイナス (突進戦法と整合)
- **sniper は最初から登場**: 全 Phase に少量ずつ (1→1→2→3→3)。Phase 1 から「遠距離も警戒する」緊張感を出す

**理由**:
- **`behavior` フィールド方式**: 「Enemy のサブクラス分け」より「stats に行動指定」のほうが、既存 1 種類クラスのまま拡張できて変更が小さい。新しい行動 (例: 後退する敵 / レーザー連射) を加えるときも `'beam'` 等を足すだけ
- **sniper の damage=0 + 'shoot'**: 接触時に Ship.update 側で `if (e.stats.behavior === 'charge')` ガードを入れることで、sniper に船が触れても無害になる。MVP の単純化
- **スタン演出を確率発火**: 毎フレーム描くと稲妻が重なって白塗りになる。確率で間引くと自然なちらつき感が出る (約 11Hz)
- **Ram のトレードオフ**: contactDps 14 (R) で敵 hp 12 (fast) を 1 秒未満で削れるが、その間 22 dps (tank) 被弾でリスクが高い。「装着すれば最強」にせず判断を残す
- **PHASES の sniper delay**: 各 Phase 開始直後ではなく数秒後に出現 → 初動は基本敵対応、その後遠距離脅威が加わるテンポ

**実装上の判断**:
- `Enemy.update(delta, ctx?)` の signature 拡張: shoot 種別が ctx.enemyBullets に push する経路。charge 種別は ctx を使わない (optional)
- 基地ヒット (charge enemy が reachedBase) でも `spawnElectricArc(enemy → base)` を出して、ship 接触と同じビジュアル言語に
- `Ship.update` の接触ループは 1 回の loop で「被ダメ + スタン演出 + 体当たり反撃」を全て処理 (距離計算が 1 回で済む)

**トレードオフ**:
- sniper の弾は基地のみ狙う (船は素通り)。船を盾にする戦法ができない。実装簡略化のため。気になれば後続で `EnemyBullet` に船判定を追加可能
- Ram モジュールは tank/boss に対して割に合わない (HP 高い → 削るのに時間 → 被ダメで死ぬ)。SR/L で実用化される設計
...
- 中身: $80 クレジット付与 (当座の便利報酬としてクレジット固定額)
...
- **`ItemInventoryScene`**: 右端「📦 アイテム」ボタンから並行 active オーバーレイ。カテゴリタブ + 所持一覧 + 詳細 + 装着フロー
...
- **中間ドロップ (Step 8)**: 各 Phase の敵を半数撃破した瞬間に $80 クレジットを付与 (Phase ごと 1 回限定)
