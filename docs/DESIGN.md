...
| **Phase 6** | アイテムシステム: レア度 4 段階 / `Inventory` (Run 揮発) / `EffectSystem` (オムニ・コア + モジュール + 時限バフ加算スタック) / ケミカル / **アイテムコード** (ITEM_CODE = 条件 wrapper、配置真実源は Code ノード自身) | 🔧 進行中 (Step 0-5 完了、ガチャ UI / リワード経路は後続) |
...
## 7. 現在の実装状態スナップショット (2026-05-22, MVP 達成 + Phase 6 Step 0-5 完了)
...
- **デバッグ用ガチャ獲得ボタン**: 各カテゴリで N/R/SR/L のランダム個体を即時付与 (本来の獲得経路は Step 6 で実装)

### Phase 6 残作業 (Step 6 以降)
- **ガチャ抽選 / 開封 UI**: `src/items/gacha.ts` は実装済 (`drawGacha(category, gachaRarity)`: 3 候補返却 → 1 つ選択) だが、**どこからも未参照**。開封 UI とインベントリ統合が未着手
