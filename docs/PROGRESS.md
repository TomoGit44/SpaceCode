...
最終更新: 2026-05-26 (Stage 拡張: 5 Stage × 各 20 Phase = 100 Phase 化)

> このドキュメントは **Phase 6 以降の現在進行中の作業** を記録する (Phase 単位 / 補追改修単位)。
> - **MVP 達成 (Phase 5) までの実装履歴 + 旧 Phase A-D**: [`HISTORY.md`](HISTORY.md)
> - **設計思想・用語・過去判断**: [`DESIGN.md`](DESIGN.md)
> - **進行中の実装プラン**: [`plans/`](plans/)
...
| 項目 | 状態 |
|---|---|
| MVP コアループ (Phase 1-5) | ✅ 完了 → [HISTORY.md](HISTORY.md) |
| Phase 6 Step 0-9 (アイテム / ガチャ / ボス / モジュール表示) | ✅ 完了 |
| 補追改修 (WAIT 統合 / ダウン状態 / 0.5x 減速 / 凍結 / sniper / オムニコア strip 他 8 件) | ✅ 完了 |
| **Stage 拡張** (5 Stage × 各 20 Phase = 100 Phase、HUD 2 段表記、Stage バナー) | ✅ 完了 (2026-05-26) |
| 残作業 | 実プレイ後バランス調整 (100 Phase ボリュームでの再計測) / 既存 6 オムニ・コアの入手経路設計 / Run 全体永続化 (オプション) |

詳細は下記の Phase 6 セクション + 補追改修セクション + 末尾の Stage 拡張セクションを参照。
...
カテゴリは 4 つ (オムニ・コア / モジュール / コードガチャ / モジュールガチャ) + 配置型の **アイテムコード** (Code 拡張)。
...
- アイテム種類追加は **data-driven** (新オムニ・コア = `OMNI_CORE_TYPES` に 1 行、モジュール = `MODULE_TYPES` に 1 行)
...
### Step 8: Wave 中間ドロップ (半数到達で $80 クレジット) ✅ 2026-05-23
**成果物**:
...
- `src/scenes/GameScene.ts`:
  ...
  - 撃破集計ループに `checkPhaseHalfReward()` を挿入: 累計撃破数が `floor(total/2)` 以上になったら $80 クレジットを付与 + 横バナー (RewardBanner) で軽量通知 (Phase ごと 1 回限定)

**設計判断**:
- **「Phase の敵を半数倒した」をトリガーに**: ガチャ報酬は Phase クリア / 敵ドロップで既に複数経路がある。中間枠は「Phase の途中で何かもらえる」リズム作りに使う
...
- **報酬は $80 クレジット固定**: 当座の便利報酬として固定額。アイテム抽選にすると Phase クリアのガチャと役割が被る
...
- 各 Phase で敵を半数倒した瞬間に「中盤ボーナス +$80」バナー
...
## 補追改修: ゲーム全体を 0.5x に減速 (2026-05-25)

### 経緯
プレイヤー要望「全体的にゲームスピードを遅くしてほしい」。プログラムを組んで動作を確認する時間的余裕がほしい (コア体験の比重を上げる) のが狙い。

### 判断
`GameScene.update` の冒頭で `delta *= GAME_SPEED` (= 0.5) を 1 行追加するのみ。
- これだけで Base / Planets / Waves / Enemies / Ships / Bullets / Effects.tick / Executor のすべてが同じテンポで遅くなる (どれも delta ベース)
- 敵移動 / 船移動 / スポーン間隔 / 採掘速度 / 弾速度 / WAIT 秒数 / ATTACK_NEAREST 持続 / 基地砲塔発射間隔 すべて整合的にスローモーション化
- **Phaser tween / camera flash / showBanner / 中央バナーイージング** などの UI 演出は `scene.time` ベースで動くため、この delta スケールに影響されない → キビキビ感が保たれる

### 成果物
- `src/config.ts`: `GAME_SPEED = 0.5` を新規 export
- `src/scenes/GameScene.ts`: import 追加、`update()` 冒頭で `delta = delta * GAME_SPEED;`

### 設計判断
- **delta スケール方式を採用**: 個別定数を全部 2 倍にする (`SHIP.moveSpeed: 100 → 50` 等) よりも、1 か所で完結し、定数の意味 (config 値 = 標準速の値) も保たれる
- **`scene.time` 系を意図的に分離**: UI フィードバック (バナー / 演出) は遅くしたくない。`delta` 系だけスケールすることで自然に分離できる
- **値 = 0.5 を定数化**: 将来「0.7x に戻したい」「Hard モードで 0.85x」等の調整は config 1 行で完結

### 検証
- typecheck / build PASS、preview console エラーなし
- 数値挙動: WAIT 5 は実時間 10 秒に伸びるが、その間の採掘量も半分のレートなので、結局 inventory は 25 → 25 (cap で頭打ち) と同じ結果。ゲーム経済は崩れない

---

## 補追改修: 準備時間中はゲーム全体を凍結 (2026-05-25)

### 経緯
プレイヤー要望「準備時間中にはゲームが動かないようにしてほしい」。Phase 開始ボタン待ちの時に Ship が動いていたり敵が残っていたりすると、編集中に集中できない。

### 判断
`GameScene.update` の冒頭、GAME_SPEED 適用より前に `if (this.waves.getState() === 'preparing') return;` を 1 行追加。これだけで:
- Ship 移動 / 採掘 / 攻撃 / 命令 tick → 停止
- 敵移動 / 接触判定 → 停止 (そもそも preparing 時点で敵は居ないことが多いが念のため)
- 弾の進行 → 停止
- 惑星リスポーンタイマー → 停止
- Effects.tick (時限バフ) → 停止
- 基地砲塔発射 → 停止
- 撃破集計 / 中盤ボーナス判定 → 停止 (実行されない)

### 影響を受けないもの
- Phaser tween (バナーイージング、HUD 開始ボタン脈動、シーンフェード)
- カメラ flash / shake
- pointer / keyboard イベント (船クリック / アイテムボタン / 開始ボタン / SPACE で開始)
- アイテムオーバーレイ / 編集オーバーレイの動作 (別シーンなので独立)

### 設計判断
- **早期 return は GAME_SPEED の前に置く**: 後だと `delta = delta * 0.5` を実行してから return することになり、意味は同じだが無駄な計算が発生する
- **waves.update も止まる (これも update スキップに含まれる)**: 元から `preparing` 状態の WaveSystem.update は何もしないので影響なし。startNextPhase() はボタン入力経由で別ルートから呼ばれる
- **stat panel の追従もスキップ**: 選択中船の値が変わらないので問題なし。ただし refresh されないので、編集オーバーレイ側で同じ静的な値を表示し続ける (実害なし)
...
## 補追改修: 遠距離敵 sniper + 体当たり強化 (スタンガン演出) + 体当たりモジュール (2026-05-25)

### 経緯
プレイヤー要望:
1. 弾で攻撃してくる敵を追加 (これまでは全敵が体当たり一辺倒)
2. 体当たり敵には「電気スタンガン」のような演出 + 攻撃力を高くする
3. プレイヤー側にも体当たり攻撃できるモジュールを追加

### 確定済み設計判断
- ダメージバンプ: **×1.5** (basic 10→15, fast 8→12, tank 15→22, boss 30→45, ENEMY_VS_SHIP.contactDps 8→12)
- sniper 出現: **Phase 1 から** (全 Phase に少量ずつ)、Phase 進行で 1 → 1 → 2 → 3 → 3
- 体当たりモジュール `mod_ram` (衝角ブレード): contactDps **N=8 / R=14 / SR=22 / L=35**、トレードオフで移動速度マイナス

### 成果物

**新規ファイル**:
- `src/entities/EnemyBullet.ts`: 基地静的座標をターゲットにする直線飛行弾。赤 + glow + 白コア。`hitsBase(baseX, baseY, baseRadius)` で衝突判定、`destroy()` で明示破棄

**Enemy 改修** (`src/entities/Enemy.ts`):
- `EnemyTickContext { enemyBullets: EnemyBullet[] }` 型を export
- `Enemy.update(delta, ctx?)` に context 追加 (shoot 種別が弾を push する経路)
- shoot 種別 (`stats.behavior === 'shoot'`) ハンドリング: `attackRange` 内なら停止 + 砲身を基地に向ける + `fireIntervalMs` ごとに `EnemyBullet` を生成 + 砲口フラッシュ
- sniper 専用描画: 外周リング + 砲身の細い長方形 + コア
- `spawnElectricArc(scene, fromX, fromY, toX, toY)` を export: 6 セグメントのジグザグ稲妻、110ms フェードアウト、シアン外側 + 白内側

**config 更新** (`src/config.ts`):
- `EnemyType` に `'sniper'` 追加、`EnemyBehavior` 型を export
- `EnemyTypeStats` に optional `behavior`/`attackRange`/`fireIntervalMs`/`bulletDamage`/`bulletSpeed`
- ENEMY_TYPES: 全種に `behavior` 設定、charge 種の damage を ×1.5、sniper 新規 (HP 25 / 速度 45 / damage 0 / 射程 280 / 1800ms 間隔 / 弾ダメ 10 / 弾速 240 / 色 0x44ddaa)
- `ENEMY_VS_SHIP.contactDps`: 8 → 12
- `PHASES`: 各 Phase に sniper を追加 (1→1→2→3→3、Phase 1 から)

**モジュール `mod_ram`** (`src/items/types/modules.ts`):
- `ModuleStatTarget` に `'contactDps'` 追加、`MODULE_STAT_LABEL.contactDps = '体当たり威力'`
- `MODULE_TYPES.mod_ram`: 衝角ブレード。`contactDps` flat (N=8/R=14/SR=22/L=35) + `moveSpeed` percent マイナス

**EffectSystem** (`src/items/effects.ts`):
- `shipContactDps(ship)`: 装着モジュールの contactDps flat 合計

**Ship.update** (`src/entities/Ship.ts`):
- 既存の敵接触ループを拡張:
  - charge 種別: 被ダメージ + 18% 確率で `spawnElectricArc(enemy → ship)`
  - sniper (damage=0 / behavior='shoot'): 接触してもダメージ無し
  - 装着 contactDps > 0: 敵に `takeDamage(dps × delta/1000)`

**GameScene** (`src/scenes/GameScene.ts`):
- `enemyBullets: EnemyBullet[]` フィールド + create/cleanup でリセット
- 敵更新ループに `{ enemyBullets }` context を渡す
- 敵が基地接触のとき charge 種別なら `spawnElectricArc(enemy → base)`
- `enemyBullets` 更新ループ + 基地ヒット判定 + 短い shake (80ms)
- 廃棄フィルタに enemyBullets を追加

### 設計判断・実装上の判断
- **EnemyBullet を独立クラス化**: 既存 Bullet はホーミング型で静的座標追従とは構造が違うため、独立クラスのほうが見通しが良い
- **sniper の `damage=0` + `behavior='shoot'` で接触素通り表現**: Ship.update 側で `if (e.stats.behavior === 'charge')` ガードを入れることで自然に実現
- **電気スタン演出は 18% 確率発火**: フレーム連発を防ぐ。約 11Hz の自然なちらつきで「スタンガン感」を出す
- **ジグザグは法線方向ジッター**: 線分を等間隔分割し中間点を法線に ±7px ランダム偏移。シンプルで稲妻らしい
- **PHASES への sniper 追加は遅延差で表現**: 基本敵が先に来て、数秒後に sniper が加わるテンポ

### ゲームの挙動
- Phase 1 開始 → basic 5 体が順次出現 → 6 秒後に sniper 1 体出現 → 射程 280px で停止して 1.8 秒ごとに弾を基地に発射
- sniper を倒すには Ship の ATTACK_NEAREST or 衝角ブレード Ram で対処
- 基本敵 / fast / tank / boss が船 or 基地に接触すると、シアン + 白の稲妻が ~110ms 表示
- 衝角ブレード装着 Ship を敵の進路に置くと接触中ずっと敵が削れる (鈍化のトレードオフ)

### 検証
- typecheck / build PASS、preview console エラーなし

### 既知の制限
- sniper の弾は基地のみを狙う (船は素通り)。船と基地の間を弾が貫通する設計上の仕様
- Ram は敵 hp が硬いほどリスクが高い (基本敵 hp 20 = N Ram で 2.5 秒、tank hp 55 = N Ram で 7 秒 → 7 秒 × 22 dps = 154 ダメージ被弾)。実プレイで R/SR 想定

---

## 補追改修: オムニ・コア 3 種追加 / 左上 Strip 化 / アイテムメニューからタブ撤去 (2026-05-25)

### 経緯
プレイヤー要望:
1. オムニコアそれぞれに固有アイコンを付け、画面左上に **常時** 横一列で表示
2. アイテムメニューからオムニコアタブを削除
3. 新コア 3 種追加:
   - 全宇宙船の攻撃力 +50%
   - 全宇宙船のエネルギー消費 -50%
   - 全宇宙船の最大 HP +50%
...
### 確定済み設計判断
- **3 新コアは Run 開始時に SR レアリティで自動装着** (スターター)。`core_attack_plus` / `core_efficiency` / `core_endurance`
- **既存 6 コア (`core_attack` / `core_thruster` / `core_drill` / `core_hull` / `core_turret` / `core_bounty`) は現状の入手経路なし**: 元々アイテムメニューのデバッグボタンのみで、本変更でそのボタンも消滅。コードベースには残置 (将来 Phase 報酬等の入手経路を追加可能)
- **`energyConsume` を `ShipStat` に追加**: デフォルト 1.0 の倍率扱い、コアで -0.5 → 0.5 倍消費 (= -50%)
- **左上 strip の常時表示**: 所持コアを横一列。重複は同アイコン 1 つにまとめて右下に ×N バッジ。hover でツールチップ (名前 / 効果 / 所持数)

### 成果物

**新規ファイル**:
- `src/ui/OmniCoreStrip.ts`: 左上常時表示 strip。`refresh()` でインベントリから再描画。各コアにそれぞれ Graphics で描画した固有シンボル (lightning / arrow / drill / shield / crosshair / dollar / battery / shield+cross / lightning+sparks)

**`src/items/itemTypes.ts`**:
- `ShipStat` に `'energyConsume'` 追加 (デフォルト 1.0 の倍率値として運用)

**`src/items/types/omniCores.ts`**:
- 3 新コア追加: `core_attack_plus` (damagePerShot +50% 固定) / `core_efficiency` (energyConsume -50% 固定) / `core_endurance` (maxHp +50% 固定)
- `rarityPercent` を全レアリティ同じ値にすることで「レアリティに依存しない固定効果」を表現 (表示用 rarity は描画のみで使う)

**`src/entities/Ship.ts`**:
- `fireAt`: `shotCost = SHIP.energyPerShot * effects.shipStat(this, 'energyConsume', 1)` で省エネコア反映
- 移動消費: `SHIP.energyConsumePerSec * consumeMul * delta/1000`

**`src/items/types/modules.ts`**:
- `ModuleStatTarget` に `energyConsume` の整合 (`MODULE_STAT_LABEL` を埋める)

**`src/scenes/GameScene.ts`**:
- create 時に 3 新コアを inventory.items に push (rarity 'SR')
- `OmniCoreStrip` を左上 (12, 76) に初期化
- ItemInventoryScene の `onChanged` callback で `omniCoreStrip.refresh()` も呼ぶ
- cleanup で strip 破棄

**`src/scenes/ItemInventoryScene.ts`**:
- `CATEGORIES` / `IMPLEMENTED` から `'omniCore'` を削除
- `selectedCategory` のデフォルトを `'module'` に変更
- `countForCategory` / `iconColorFor` / `subtextFor` / `categoryItems` / `displayName` / `debugGrant` から omni 分岐を撤去
- `renderCoreDetail` メソッド削除 (詳細表示は strip の hover ツールチップに集約)
- `omniCores` モジュールの import を撤去

### 設計判断・実装上の判断
- **`energyConsume` を倍率にする**: 加算スタック (`base * (1 + Σ percent)`) の枠で `-0.5` → `1 * 0.5 = 0.5 倍消費` が自然に表現できる。新ヘルパ不要、既存 `effects.shipStat` がそのまま使える
- **スターター方式を採用**: 「左上に常時表示」「アイテムメニューから撤去」「3 新コア追加」を整合させる最短ルート。既存 6 コアの入手経路は別 Issue として保留
- **strip は HUD 配下の独立コンポーネント**: 既存 `HUD` を太らせず、`OmniCoreStrip` 単独で完結。`refresh()` API で外部からインベントリ変化を通知
- **アイコンは画像なしで Graphics 描画**: 9 種それぞれにシンボル (lightning/arrow/drill/shield+/cross/crosshair/$/battery 等)。プロジェクト方針の「画像アセット不使用」を維持
- **既存 6 コアのコードは残置**: 削除すると将来再導入時に書き直しコストが発生。`OMNI_CORE_TYPES` から外せばインベントリにも入らないので無影響

### ゲームの挙動
- 新規 Run 開始 → 画面左上に 3 アイコン (赤+/緑バッテリー/緑シールド+) が SR 色 (紫) のボーダーで表示
- 全宇宙船は 攻撃力 ×1.5 / エネルギー消費 ×0.5 / 最大 HP ×1.5 の常時バフ下で開始
- アイコンに hover → 名前 + 効果 + 所持数のツールチップ
- アイテムメニュー (📦 アイテム) を開いてもオムニ・コアタブは表示されない (デフォルトは「モジュール」)
- ダメージバランス: 攻撃力 +50% は基地砲塔には影響なし (これは `base` ターゲットの core_turret 担当)。船の damagePerShot のみが対象

### 検証
- typecheck / build PASS、preview console エラーなし

### 既知の制限 / 後続課題
- 既存 6 コア (`core_attack` / `core_thruster` / `core_drill` / `core_hull` / `core_turret` / `core_bounty`) は現状の入手経路なし。`OMNI_CORE_TYPES` から定義は残置するが Inventory には自動投入されない → **TODO: ガチャ排出 or Phase 報酬等の新規入手経路を設計**
- スターター 3 コアは Run 毎に固定で SR 装着。レアリティ抽選はしていない (固定効果のため意味なし)
- 体感バランスはまだ調整していない (新コアの恒常 +50% / -50% は強力)。実プレイ後に config 調整想定
...
| `src/config.ts` | ✅ | Phase 6: `COLORS.rarity*` 追加。Step 7: `EnemyType` に `boss` 追加 + `PHASES[4]` に boss spec。2026-05-25: `SHIP.refuelCost`/`repairCost` + `GAME_SPEED` (0.5) + `EnemyType` に `sniper` 追加 + 体当たり damage ×1.5 + `ENEMY_VS_SHIP.contactDps` 12 + PHASES に sniper 配備 |
| `src/scenes/BootScene.ts` | ✅ | 変更予定なし |
| `src/scenes/MenuScene.ts` | ✅ | Phase 5 演出済 |
| `src/scenes/GameScene.ts` | ✅ | Phase 6: `overlayDepth` / `Inventory` + `EffectSystem` / Ship 破壊時のモジュール返却 / `applyMaxStats`。Step 6: `grantPhaseClearGacha` + `rollEnemyDropGacha`。Step 7-8: ボス出現バナー / `phaseKillCount` + `checkPhaseHalfReward`。**2026-05-25**: `enemyBullets[]` + 基地ヒット処理 + 体当たり時の電気スタン演出 |
...
| `src/entities/Enemy.ts` | ✅ | Phase 4: 3 種化、Phase 6 Step 7: boss 専用描画、**2026-05-25**: sniper (shoot behavior) + 砲身描画 + spawnElectricArc helper |
| `src/entities/EnemyBullet.ts` | ✅ | **2026-05-25 新規**: sniper の弾。基地静的座標を直線追従、hitsBase で衝突判定 |
| `src/entities/Bullet.ts` | ✅ | 基地砲塔/Ship 共用 |
| `src/entities/Planet.ts` | ✅ | Phase 4: 60s リスポーン |
| `src/entities/Ship.ts` | ✅ | Phase 6: `id` 発番 / `maxHp`/`maxEnergy`/`inventoryCap` 可変 / `applyMaxStats` / `heal` / `fireAt` が複数弾 / stat 参照を `effects.shipStat` 経由。**2026-05-25**: `ShipState` に `downed` 追加、`takeDamage` で自動 die 撤廃、HP 0 = ダウン。接触ループに体当たりモジュール反撃 + charge 種別接触で電気スタン演出 |
...
## Stage 拡張 — 5 Stage × 各 20 Phase = 100 Phase 化 (2026-05-26) ✅ 完了

### 目標
1 Run = 5 Phase は短すぎて Phase 6 のアイテム成長要素が活きないため、ボリュームを **5 Stage × 20 Phase = 合計 100 Phase** に拡張する。Stage 区切りは演出のみで遊びは中断しない (連続プレイ)。

### 成果物
- **`src/config.ts`** — `STAGE` を `{ totalPhases: 100, totalStages: 5, phasesPerStage: 20 }` に拡張。`PHASES` を 100 要素手書き (Stage 単位コメント区切り)。ボスは Phase 20/40/60/80/100 の 5 箇所。
- **`src/systems/WaveSystem.ts`** — Stage 派生 API (`getStageNumber` / `getPhaseInStage` / `getTotalStages` / `getPhasesPerStage` / `isBossPhase` / `isStageBoundary`) と `stageStart` イベントを追加。`startPhase()` で Stage 先頭 Phase なら `stageStart` を `phaseStart` より先に emit。
- **`src/ui/HUD.ts`** — `setPhase` を `setStageAndPhase(stage, totalStages, phaseInStage, phasesPerStage)` に置き換え (中央 HUD は `STAGE · PHASE` の 2 段表記、値は `3 / 5 · 12 / 20`)。`showStartButton` も Stage 込み表記 (`▶ Stage X / Phase Y / 20 開始`) に変更。`showBanner` に色引数を追加 (Stage バナー用)。
- **`src/scenes/GameScene.ts`** — `waves.on('stageStart')` 購読で「STAGE N」accent バナー + camera flash。Stage 先頭 Phase では Phase バナーを 1.05s 遅らせて重なり回避。`updateStatusText` の preparing 文言、ガチャ報酬付与は通し Phase 番号 (1-100) 維持、HUD/バナー表示は Stage 内 Phase 番号 (1-20)。
- **`src/scenes/VictoryScene.ts`** — タイトル `STAGE CLEAR` → `ALL STAGES CLEAR` (フォントサイズ 96→76px)、サブテキスト「基地を守り抜いた」→「全 5 Stage を踏破した」。

### 難易度カーブ (PHASES 100 要素手書き方針)
- **Stage 1 (1-20)**: basic 中心。Phase 4 sniper / 9 hunter / 15 fast 段階導入。20 でボス。
- **Stage 2 (21-40)**: fast/sniper/hunter 標準化、Phase 35 で tank 解禁、40 でボス。
- **Stage 3 (41-60)**: 全敵混在、Stage 2 比で密度 +30%、60 でボス。
- **Stage 4 (61-80)**: 出現密度さらに上昇 + 特徴 Phase 3 種 (65 sniper ラッシュ / 70 tank ラッシュ / 75 fast 速攻波)、80 でボス。
- **Stage 5 (81-100)**: 全種高頻度・intervalMs 最短水準 + 特徴 Phase 3 種 (85 混沌波 / 90 tank 多め詰め / 95 終末ラッシュ)、Phase 100 はボス早期登場 (12s) で雑魚と被る総力戦。

### 設計判断・プランからの逸脱
- **Stage は派生概念に統一**: WaveSystem の状態機械は Phase 単位の線形進行のまま (Stage の状態機械は追加しない)。`stage = floor(phaseIndex / phasesPerStage) + 1` で計算するヘルパに留めた。
- **ガチャ報酬の振り分け**: `phaseRewardCategory(phaseNumber)` (奇数=code/偶数=module) は通し Phase 番号 (1-100) で動かす。100 Phase でも自然に交互振り分けされる。
- **ボス判定の汎用化**: `isBossPhase()` を新設 (enemySpecs に boss が含まれるか) し、Phase 5 末尾固定の暗黙仮定を排除。実装上の現行ロジック (`enemySpawned` で `enemy.type === 'boss'` 検知) は変更不要だったため、isBossPhase は今後の追加演出用 API として残置。
- **HUD 表記**: ユーザー回答は「Stage X / Phase Y」だが、分母 (Total Stages / Phases per Stage) を残したほうが進捗感が出るため `3 / 5 · 12 / 20` の 2 段並列とした。

### 完了時の検証
- `npm run typecheck`: PASS / `npm run build`: 約 6.9s、gzip 377KB (PHASES データ追加で約 +0.3KB)
- preview_console_logs (level: error): エラー無し

### 残課題 (実プレイ後)
- 100 Phase ボリュームでのバランス再調整 (Stage 1 は緩めに振っているが、後半の密度上昇が急すぎる/緩すぎる可能性)
- アイテム成長カーブが 100 Phase に対して十分か (ガチャは Phase クリアごと + fast/tank ドロップ + ボス SR 確定の現行どおり)
- ボリュームによる集中力切れ対策 (Stage 末尾で中継 Victory 画面を入れる選択肢は今回見送り、各 Phase の手動開始制で休憩を取れる前提)
