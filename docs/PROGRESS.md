...
最終更新: 2026-05-23 (Phase 6 Step 0-6 完了 — ガチャ系統まで実装)
...
- **Run リワード経路の拡張**: ボス的な敵 / Wave 中間ドロップ等の追加 (現状: Phase クリア + fast/tank ドロップのみ)
- **ProgramEditorScene 内モジュール表示** (仕様 §7.4): ItemInventoryScene で完結するため後回し
- **実プレイ後バランス調整**: Phase 6 アイテムが入った状態での再計測 (ガチャ排出レアリティの重み・敵ドロップ率・アイテム効果値)
...
| `src/scenes/GameScene.ts` | ✅ | Phase 6: `overlayDepth` カウンタ / `Inventory` + `EffectSystem` 保有 / `applyChemical` / Ship 破壊時のモジュール返却 / Ship 購入時 `applyMaxStats`。Step 6 で `grantPhaseClearGacha` + `rollEnemyDropGacha` を追加 |
