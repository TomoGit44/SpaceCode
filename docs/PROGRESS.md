...
最終更新: 2026-05-24 (Phase 6 Step 0-9 + コード体系改修 — MINE/DEPOSIT/WAIT_UNTIL_FULL を WAIT に統合)
...
| `src/config.ts` | ✅ | Phase 6: `COLORS.rarity*` 追加。Step 7: `EnemyType` に `boss` 追加 + `PHASES[4]` に boss spec |
...
| `src/scenes/ProgramEditorScene.ts` | ✅ | Phase 6: `inventory`/`getShips` 受領、ITEM_CODE 配置 + 残数管理。Step 9: 装着モジュール read-only チップ表示 |
