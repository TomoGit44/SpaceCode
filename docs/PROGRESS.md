...
| `src/scenes/GameScene.ts` | ✅ | Phase 6: `overlayDepth` / `Inventory` + `EffectSystem` / `applyChemical` / Ship 破壊時のモジュール返却 / `applyMaxStats`。Step 6: `grantPhaseClearGacha` + `rollEnemyDropGacha`。Step 7-8: ボス出現バナー / `phaseKillCount` + `checkPhaseHalfReward`。**2026-05-25**: `enemyBullets[]` + 基地ヒット処理 + 体当たり時の電気スタン演出 |
