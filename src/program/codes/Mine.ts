...
export function tickMine(
  code: Extract<Code, { type: 'MINE' }>,
  ship: Ship,
  world: ShipWorld
): CodeStepResult {
  const planet = resolvePlanet(code.target, world);
  if (!planet) {
    return { status: 'blocked', reason: `${LOCATION_LABELS[code.target]} が存在しない` };
  }
  if (ship.isInventoryFull()) {
    return { status: 'done' };
  }
  if (planet.depleted) {
    return { status: 'blocked', reason: `${LOCATION_LABELS[code.target]} がリスポーン中` };
  }
  ship.mineAt(planet);
